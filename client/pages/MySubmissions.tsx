...
export default function MySubmissions() {
  ...
  const mySubmissions = useMemo(() => {
    ...
    return submissions.filter((s) =>
      String(s.citizen).toLowerCase().includes(String(name).toLowerCase()),
    );
  }, [name, submissions]);
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-semibold">
                Interactive submissions area
              </h2>
              <p className="text-sm text-gray-600">
                {name
                  ? `Showing submissions for ${name}`
              ...
              <tbody>
                {mySubmissions.length === 0 ? (
                  <tr>
                    ...
                    >
                      No submissions found for {name}.{" "}
                      <Link
                        to="/submit"
                        className="text-jansoch-blue underline"
                      >
                        Create one
                      </Link>
                      .
                  ...
                  mySubmissions.map((r) => (
                    ...
                    >
                      ...
                      <td className="py-3 text-sm">
                        <span
                          className={`inline-block px-3 py-1 rounded-full text-xs ${r.status === "Implemented" ? "bg-green-100 text-green-800" : r.status === "Under Review" ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-800"}`}
                        >
                          {r.status}
                        </span>
                      </td>
